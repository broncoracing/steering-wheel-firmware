...
static const int txb = 8; // with default settings, all buffers before this are
                          // consumed by the FIFO
...
FlexCAN::FlexCAN(uint32_t baud) {
  ...
  if (50000 == baud) {
    FLEXCAN0_CTRL1 =
        (FLEXCAN_CTRL_PROPSEG(2) | FLEXCAN_CTRL_RJW(1) | FLEXCAN_CTRL_PSEG1(7) |
         FLEXCAN_CTRL_PSEG2(3) | FLEXCAN_CTRL_PRESDIV(19));
  } else if (100000 == baud) {
    FLEXCAN0_CTRL1 =
        (FLEXCAN_CTRL_PROPSEG(2) | FLEXCAN_CTRL_RJW(1) | FLEXCAN_CTRL_PSEG1(7) |
         FLEXCAN_CTRL_PSEG2(3) | FLEXCAN_CTRL_PRESDIV(9));
  } else if (250000 == baud) {
    FLEXCAN0_CTRL1 =
        (FLEXCAN_CTRL_PROPSEG(2) | FLEXCAN_CTRL_RJW(1) | FLEXCAN_CTRL_PSEG1(7) |
         FLEXCAN_CTRL_PSEG2(3) | FLEXCAN_CTRL_PRESDIV(3));
  } else if (500000 == baud) {
    FLEXCAN0_CTRL1 =
        (FLEXCAN_CTRL_PROPSEG(2) | FLEXCAN_CTRL_RJW(1) | FLEXCAN_CTRL_PSEG1(7) |
         FLEXCAN_CTRL_PSEG2(3) | FLEXCAN_CTRL_PRESDIV(1));
  } else if (1000000 == baud) {
    FLEXCAN0_CTRL1 =
        (FLEXCAN_CTRL_PROPSEG(2) | FLEXCAN_CTRL_RJW(0) | FLEXCAN_CTRL_PSEG1(1) |
         FLEXCAN_CTRL_PSEG2(1) | FLEXCAN_CTRL_PRESDIV(1));
  } else { // 125000
    FLEXCAN0_CTRL1 =
        (FLEXCAN_CTRL_PROPSEG(2) | FLEXCAN_CTRL_RJW(1) | FLEXCAN_CTRL_PSEG1(7) |
         FLEXCAN_CTRL_PSEG2(3) | FLEXCAN_CTRL_PRESDIV(7));
  }
...
void FlexCAN::begin(const CAN_filter_t &mask) {
  ...
  if (mask.ext) {
    FLEXCAN0_RXFGMASK = ((mask.rtr ? 1 : 0) << 31) |
                        ((mask.ext ? 1 : 0) << 30) |
                        ((mask.id & FLEXCAN_MB_ID_EXT_MASK) << 1);
  } else {
    FLEXCAN0_RXFGMASK = ((mask.rtr ? 1 : 0) << 31) |
                        ((mask.ext ? 1 : 0) << 30) |
...
void FlexCAN::setFilter(const CAN_filter_t &filter, uint8_t n) {
  if (8 > n) {
    if (filter.ext) {
      ...
      FLEXCAN0_IDFLT_TAB(n) = ((filter.rtr ? 1 : 0) << 31) |
                              ((filter.ext ? 1 : 0) << 30) |
                              (FLEXCAN_MB_ID_IDSTD(filter.id) << 1);
    }
  }
}
...
int FlexCAN::write(const CAN_message_t &msg) {
  ...
  FLEXCAN0_MBn_WORD0(buffer) =
      (msg.buf[0] << 24) | (msg.buf[1] << 16) | (msg.buf[2] << 8) | msg.buf[3];
  FLEXCAN0_MBn_WORD1(buffer) =
  ...
  if (msg.ext) {
    FLEXCAN0_MBn_CS(buffer) = FLEXCAN_MB_CS_CODE(FLEXCAN_MB_CODE_TX_ONCE) |
                              ...
                              FLEXCAN_MB_CS_SRR | FLEXCAN_MB_CS_IDE;
  } else {
    FLEXCAN0_MBn_CS(buffer) = FLEXCAN_MB_CS_CODE(FLEXCAN_MB_CODE_TX_ONCE) |
                              FLEXCAN_MB_CS_LENGTH(msg.len);
  }
